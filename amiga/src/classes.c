...
extern int ah, bf, ot, tt, vf, lf, tc, rf, of, bl, vv, ss, pf, uu, vv, ww, startnewgame, gamewin;
extern char *dim, *bright, *bell, sot[128];
...
if (temp == 52)
{
	...
	if (infile == NULL)
	{
		sprintf (sot, "%sError. Unable to load file!%s\n", bright, dim);
		printw (sot);
		sleep(1);
		close (gamewin);
		exit(1);
...
{
	splash();
	sprintf (sot, "%sOnly shaded questions require use of the RETURN key!\n", bell);
	printw (sot);
	dd = 2;
	uu = 6;
	ww = 10;
	sprintf (sot, "\nAll right, bold one.\n");
	printw (sot);
	sprintf (sot, "%sYou may be (D)warf, (E)lf, Hu(M)an or (H)obbit.\n", pad);
	printw (sot);
	sprintf (sot, "\nCommand #%i? ", tt);
	printw (sot);
	++tt;
	...
	printw (sot);
	sprintf (sot, "%s%s = %i %s = %i %s = %i\n", pad, strength, uu, intelligence, vv, dexterity, ww);
	printw (sot);
	sprintf (sot, "%sand %i other points to allocate as you wish.\n\n", pad, ot);
	printw (sot);
	...
	printw (sot);
	sprintf (sot, "%s(N)one (L)eather-10 (C)hainmail-20 (P)late-30\n", pad);
	printw (sot);
	sprintf (sot, "\nCommand #%i? ", tt);
	printw (sot);
	++tt;
	...
	printw (sot);
	sprintf (sot, "%s(N)one (D)agger-10 (M)ace-20 (S)word-30\n",pad);
	printw (sot);
	sprintf (sot, "\nCommand #%i? ", tt);
	printw (sot);
	++tt;
	...
	if (gg >= 1)
	{
		...
		do
		{
			...
			printw (sot);
			sprintf (sot, "          %sHow many do you want? ", bright);
			printw (sot);
			scanw (&buf);
			sprintf (sot, "%s", dim);
			printw (sot);
	...
	printw (sot);
	sprintf (sot, "%s   Castle level numbers increase as you go DOWN!%s\n\n", bright, dim);
	printw (sot);
	border ("-", 52);
