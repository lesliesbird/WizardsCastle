...
while (quit == 0)
{
	...
	if (tt - (tt/500) * 500 == 0)
	{
		printf ("\n");
		border ("-", 52);
		printf ("%sYou hear a voice whisper%sZOT!\n", qpad, dots);
		bell();
		border ("-", 52);
		}
	wc = 0;
	...
	if (((index <= 6) || (index >= 11)) && (index <= 12))
	{
		...
		if (roll(6) == 1)
		{
			n = roll(35);
			isfrog = (n + 4) / 5;
			printf ("\n%s%sOn the floor is a%s %s!%s\n", pad, bright, frog[n - 1], frog[34 + isfrog], dim);
			printf ("    ");
			bell();
			border ("-", 52);
			}
	...
	if (index == 9) /* a warp! */
	{
		if ((o[0] == x_axis) && (o[1] == y_axis) && (o[2] == z_axis) && (temp == 33) && (of == 0))
		{
			...
			printf ("%sThe RUNESTAFF has disappeared!\n", qpad);
			bell();
			justfound = 1;
			rf = 0;
			of = 1;
...
check_events(void)
{
	...
	for (n = 1; n<= 3; ++n)
	{
		...
		if (m)
		{
			printf("\n");
			border ("-", 52);
			printf ("%sYou just received the curse of %s\n", pad, curse[n - 1]);
			border ("-", 52);
			bell();
			}
		}
...
bell(void)
{
	PlaySound("Default Beep", NULL, SND_ASYNC);
	return 0;
	}
